...
const RoomCancellationApproval = ({ onSuccess, onCancel }) => {
  const { user } = useAuth();
  ...
  // Mock data cho các đơn hủy phòng
  const mockCancellationRequests = [
    {
      ...
      cancellation: {
        ...
        reason: 'Tốt nghiệp sớm',
        expectedMoveOutDate: '2024-07-01',
        refundAmount: 800000,
        penaltyFee: 0,
        finalRefundAmount: 800000
      },
      status: 'pending',
    ...
    },
    {
      ...
      cancellation: {
        ...
        expectedMoveOutDate: '2024-07-15',
        refundAmount: 6000000,
        penaltyFee: 1200000,
        finalRefundAmount: 4800000
      },
      status: 'pending',
    ...
    },
    {
      ...
      cancellation: {
        ...
        expectedMoveOutDate: '2024-07-30',
        refundAmount: 8000000,
        penaltyFee: 800000,
        finalRefundAmount: 7200000
      },
      status: 'pending',
    ...
    },
    {
      ...
      cancellation: {
        ...
        reason: 'Tốt nghiệp',
        expectedMoveOutDate: '2024-07-10',
        refundAmount: 1200000,
        penaltyFee: 0,
        finalRefundAmount: 1200000
      },
      status: 'approved',
    ...
    },
    {
      ...
      cancellation: {
        ...
        reason: 'Chuyển về quê',
        expectedMoveOutDate: '2024-07-20',
        refundAmount: 8000000,
        penaltyFee: 1600000,
        finalRefundAmount: 6400000
      },
      status: 'rejected',
      createdAt: '2024-07-01T11:30:00Z',
      rejectedAt: '2024-07-02T09:15:00Z',
      rejectedBy: 'Admin002',
      rejectionReason: 'Thiếu giấy tờ chứng minh',
      documents: {
        hometownDocument: false,
        moveOutRequest: true,
        roomConditionReport: false
      }
    },
    {
      ...
      cancellation: {
        ...
        reason: 'Tốt nghiệp sớm',
        expectedMoveOutDate: '2024-07-25',
        refundAmount: 12000000,
        penaltyFee: 0,
        finalRefundAmount: 12000000
      },
      status: 'pending',
    ...
    },
    {
      ...
      cancellation: {
        ...
        reason: 'Chuyển trường',
        expectedMoveOutDate: '2024-08-01',
        refundAmount: 8000000,
        penaltyFee: 800000,
        finalRefundAmount: 7200000
      },
    ...
    },
    {
      ...
      cancellation: {
        ...
        expectedMoveOutDate: '2024-08-05',
        refundAmount: 12000000,
        penaltyFee: 0,
        finalRefundAmount: 12000000
      },
  ...
  useEffect(() => {
    // Load cancellation requests from localStorage and combine with mock data
  ...
  const handleApproveSelected = () => {
    ...
    // Update requests
    const updatedRequests = cancellationRequests.map(request => {
    ...
    approvedRequests.forEach(request => {
      const refundBill = {
        id: `REFUND_${request.id}`,
        studentId: request.studentId,
        studentName: request.studentName,
        studentEmail: request.studentEmail,
        type: 'refund',
        amount: request.cancellation.finalRefundAmount,
        description: `Hoàn tiền hủy phòng - ${request.currentRoom.roomNumber}`,
        status: 'pending',
      ...
      // Save refund bill to localStorage
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="w-full px-4 py-8">
        <div className="bg-white rounded-lg shadow-lg p-8">
          <PageHeader
            title="Duyệt đơn hủy phòng KTX"
            subtitle="Quản lý và duyệt các đơn hủy phòng ký túc xá"
            showClose={true}
            onClose={onCancel}
          />
            ...
            <div className="bg-blue-50 p-4 rounded-lg">
              <div className="flex items-center">
                ...
                <div className="ml-4">
                  ...
                  <p className="text-2xl font-bold text-blue-900">{cancellationRequests.length}</p>
                </div>
              </div>
            </div>
                        ...
                        <tr key={request.id} className="hover:bg-gray-50">
                          ...
                          <td className="px-6 py-4 whitespace-nowrap">
                            <div>
                              ...
                              <div className="text-sm text-gray-500">{request.currentRoom.building}</div>
                              <div className="text-sm text-gray-500">{request.currentRoom.roomType}</div>
                            </div>
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap">
                            <div className="text-sm text-gray-900">{request.cancellation.reason}</div>
                            <div className="text-sm text-gray-500">
                              Dự kiến: {formatDate(request.cancellation.expectedMoveOutDate)}
                            </div>
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap">
                            <div className="text-sm font-medium text-gray-900">
                              {formatCurrency(request.cancellation.finalRefundAmount)}
                            </div>
                            {request.cancellation.penaltyFee > 0 && (
                              <div className="text-sm text-red-500">
                                Phí phạt: {formatCurrency(request.cancellation.penaltyFee)}
                              </div>
                            )}
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap">
      ...
      {showDetailModal && selectedRequest && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 max-w-4xl w-full mx-4 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-xl font-bold text-gray-800">Chi tiết đơn hủy phòng</h2>
              <Button
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              {/* Room Information */}
              <div className="bg-gray-50 p-4 rounded-lg">
                <h3 className="text-lg font-semibold text-gray-800 mb-4">Thông tin phòng</h3>
                <div className="space-y-2">
              ...
              <div className="bg-gray-50 p-4 rounded-lg">
                <h3 className="text-lg font-semibold text-gray-800 mb-4">Thông tin hủy phòng</h3>
                <div className="space-y-2">
                  <div><span className="font-medium">Ngày yêu cầu:</span> {formatDate(selectedRequest.cancellation.requestDate)}</div>
                  <div><span className="font-medium">Lý do:</span> {selectedRequest.cancellation.reason}</div>
                  <div><span className="font-medium">Dự kiến trả phòng:</span> {formatDate(selectedRequest.cancellation.expectedMoveOutDate)}</div>
                  <div><span className="font-medium">Số tiền hoàn:</span> {formatCurrency(selectedRequest.cancellation.refundAmount)}</div>
                  {selectedRequest.cancellation.penaltyFee > 0 && (
                    <div><span className="font-medium">Phí phạt:</span> {formatCurrency(selectedRequest.cancellation.penaltyFee)}</div>
                  )}
                  <div><span className="font-medium">Hoàn thực tế:</span> {formatCurrency(selectedRequest.cancellation.finalRefundAmount)}</div>
                </div>
              </div>
              ...
              <div className="flex items-center justify-end space-x-4 mt-6 pt-6 border-t">
                <Button
                  onClick={() => {
                    ...
                    if (rejectionReason) {
                      const updatedRequests = cancellationRequests.map(req => 
                      ...
                      alert('Đã từ chối đơn hủy phòng!');
                    }
                  }}
                  variant="danger"
                >
                ...
                <Button
                  onClick={() => {
                    ...
                    // Create refund bill
                    const refundBill = {
                      id: `REFUND_${selectedRequest.id}`,
                      studentId: selectedRequest.studentId,
                      studentName: selectedRequest.studentName,
                      studentEmail: selectedRequest.studentEmail,
                      type: 'refund',
                      amount: selectedRequest.cancellation.finalRefundAmount,
                      description: `Hoàn tiền hủy phòng - ${selectedRequest.currentRoom.roomNumber}`,
                      status: 'pending',
                      createdAt: new Date().toISOString(),
                    ...
                    setShowDetailModal(false);
                    alert('Đã duyệt đơn hủy phòng thành công!');
                  }}
                  variant="success"
                >
                  Duyệt
...
export default RoomCancellationApproval;
