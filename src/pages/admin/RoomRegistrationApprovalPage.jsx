...
const RoomRegistrationApprovalPage = ({ onSuccess, onCancel }) => {
  ...
  const loadRoomRegistrations = async () => {
    try {
      ...
      if (Array.isArray(data)) {
        ...
        setRegistrationRequests(sorted);
      ...
      console.error('Error loading room registrations:', error);
      console.error('Error response:', error.response);
      const errorMessage = error.response?.data?.message || error.message || 'Không thể tải danh sách đơn đăng ký';
  ...
  const currentRequests = registrationRequests;
  ...
  return (
    <PageLayout
      title="Duyệt đơn đăng ký KTX"
      subtitle="Quản lý và duyệt các đơn đăng ký ký túc xá của sinh viên"
      showClose={true}
      onClose={onCancel}
    >
      ...
      <LoadingState
        isLoading={loading}
        isEmpty={!loading && registrationRequests.length === 0}
        emptyState={
      ...
      >
        <ModalBody className="max-h-[calc(105vh-200px)] overflow-y-auto">
          <div className="space-y-6">
            ...
            <div>
              <h3 className="text-lg font-semibold text-gray-800 mb-3">Thông tin đăng ký</h3>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700">Vị trí phòng</label>
